...
const reportPath = path.join(outDir, "seo-weekly-apply-report.md");
...
const STEPS = [
  ...
  { label: "Apply programmatic promotions", cmd: "npm", args: ["run", "promote:programmatic-index:apply"] },
  { label: "Fix checklist silo links", cmd: "npm", args: ["run", "seed:fix-checklist-silo"] },
  { label: "Silo coverage audit", cmd: "npm", args: ["run", "audit:silo-coverage"] },
  { label: "Commit promotion snapshot", cmd: "npm", args: ["run", "promote:programmatic-index:commit"] },
...
function buildReport(results) {
  ...
  const lines = [];
  lines.push("# SEO Weekly Apply Runner Report");
  lines.push("");
...
function main() {
  ...
  console.log("=== SEO weekly apply runner ===");
