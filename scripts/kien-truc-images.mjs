...
export const KIEN_TRUC_IMAGE_ALTS = [
...
export function resolveKienTrucArticleSlug(slug) {
  if (!slug) return null;
...
export function kienTrucThumbnailPath(slug) {
  ...
  const entry = resolved ? KIEN_TRUC_ARTICLE_THUMBNAILS[resolved] : null;
  return entry ? `${KIEN_TRUC_DIR}/${entry.file}` : null;
}

export function getKienTrucThumbnailAlt(slug) {
  const resolved = resolveKienTrucArticleSlug(slug);
  const entry = resolved ? KIEN_TRUC_ARTICLE_THUMBNAILS[resolved] : null;
...
export function isKienTrucSlug(slug) {
  return Boolean(resolveKienTrucArticleSlug(slug));
}
