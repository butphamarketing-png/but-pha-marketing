/**
 * Seed 500 bài từ khóa long-tail batch 6.
 * Chạy: npm run seed:keywords-500-batch6
 */
import dotenv from "dotenv";
import { createClient } from "@supabase/supabase-js";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { KEYWORDS_500_BATCH6 } from "./seo-keywords-500-batch6.mjs";
...
let targets = KEYWORDS_500_BATCH6.filter((e) => !dbSlugs.has(e.slug));

if (onlySlug) {
  targets = KEYWORDS_500_BATCH6.filter((e) => e.slug === onlySlug);
  if (!targets.length) {
    console.error(`Slug "${onlySlug}" không có trong KEYWORDS_500_BATCH6.`);
    process.exit(1);
  }
}
...
console.log(`=== Seed 500 từ khóa batch 6 ===`);
console.log(`Batch: ${KEYWORDS_500_BATCH6.length} | DB: ${dbSlugs.size} | Mới: ${targets.length}`);
...
for (let i = 0; i < targets.length; i++) {
  ...
  try {
    const article = upgradeArticle(row, i + 2500);
    if (dryRun) {
      created++;
      continue;
    }
    ...
    if (!quiet && ((i + 1) % 50 === 0 || i === targets.length - 1)) {
