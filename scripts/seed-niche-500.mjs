/**
 * Seed ~500 bài tin tức ngành nghề ngách (giặt sấy, vựa sầu riêng…).
 * Chạy: npm run seed:niche-500
 *       npm run seed:niche-500 -- --dry-run
 *       npm run seed:niche-500 -- --limit=5
 *       npm run seed:niche-500 -- --slug=thiet-ke-website-giat-say
 */
import dotenv from "dotenv";
import { createClient } from "@supabase/supabase-js";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { NICHE_500_KEYWORDS } from "./seo-niche-500-keywords.mjs";
...
let targets = NICHE_500_KEYWORDS.filter((e) => !dbSlugs.has(e.slug));

if (onlySlug) {
  targets = NICHE_500_KEYWORDS.filter((e) => e.slug === onlySlug);
  if (!targets.length) {
    console.error(`Slug "${onlySlug}" không có trong niche 500.`);
    process.exit(1);
  }
}

const already = NICHE_500_KEYWORDS.length - targets.length;
targets = targets.slice(0, limit);

console.log(`=== Seed niche 500 ngành nghề ===`);
console.log(`Catalog: ${NICHE_500_KEYWORDS.length} | Đã có DB: ${already} | Sẽ xử lý: ${targets.length} | dry-run: ${dryRun ? "YES" : "NO"}`);
