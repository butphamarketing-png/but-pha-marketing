import { CO_KHI_ARTICLE_THUMBNAILS, validateSeoKeywordPlacement } from "./seo-article-helpers.mjs";
...
const ARTICLES = [REWRITE_THIET_KE_WEBSITE_CO_KHI, REWRITE_THIET_KE_WEBSITE_GIA_CONG_CNC];
const EXPECTED_IMAGES = 5;
let failed = false;

for (const article of ARTICLES) {
  const images = [...article.content.matchAll(/src="(\/tin-tuc\/co-khi\/[^"]+)"/g)].map((m) => m[1]);
  const otherTinTuc = [...article.content.matchAll(/src="(\/tin-tuc\/(?!co-khi)[^"]+)"/g)].map((m) => m[1]);
  ...
  const thumb = CO_KHI_ARTICLE_THUMBNAILS[article.slug];
...
console.log("All co-khi articles pass SEO + 5 images check.");
