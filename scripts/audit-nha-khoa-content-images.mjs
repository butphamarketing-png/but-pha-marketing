import { NHA_KHOA_ARTICLE_THUMBNAILS, validateSeoKeywordPlacement } from "./seo-article-helpers.mjs";
...
const ARTICLES = [REWRITE_THIET_KE_WEBSITE_NHA_KHOA, REWRITE_THIET_KE_WEBSITE_NHA_KHOA_NIENG_RANG];
const EXPECTED_IMAGES = 5;
let failed = false;

for (const article of ARTICLES) {
  const images = [...article.content.matchAll(/src="(\/tin-tuc\/nha-khoa\/[^"]+)"/g)].map((m) => m[1]);
  const otherTinTuc = [...article.content.matchAll(/src="(\/tin-tuc\/(?!nha-khoa)[^"]+)"/g)].map((m) => m[1]);
  ...
  const thumb = NHA_KHOA_ARTICLE_THUMBNAILS[article.slug];
...
console.log("All nha-khoa articles pass SEO + 5 images check.");
