...
import {
  ...
  LEGACY_SLUG_DA_NANG,
} from "./seo-rewrite-thiet-ke-website-da-nang.mjs";
...
const article = REWRITE_THIET_KE_WEBSITE_DA_NANG;
const check = validateSeoKeywordPlacement({
  ...
  imageAlts: ["Thiết kế website Đà Nẵng cho du lịch và dịch vụ miền Trung"],
});
...
const { data: byLegacySlug } = byNewSlug
  ? { data: null }
  : await supabase.from("news").select("id,slug").eq("slug", LEGACY_SLUG_DA_NANG).maybeSingle();
