/**
 * Seed 1000 từ khóa khách hàng hay tìm.
 * Chạy: npm run seed:customer-kw-1000
 *       npm run seed:customer-kw-1000 -- --dry-run --limit=3
 */
import dotenv from "dotenv";
import { createClient } from "@supabase/supabase-js";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { CUSTOMER_KEYWORDS_1000 } from "./seo-customer-keywords-1000.mjs";
...
let targets = CUSTOMER_KEYWORDS_1000.filter((e) => !dbSlugs.has(e.slug));

if (onlySlug) {
  targets = CUSTOMER_KEYWORDS_1000.filter((e) => e.slug === onlySlug);
  if (!targets.length) {
    console.error(`Slug "${onlySlug}" không có trong customer-kw-1000.`);
    process.exit(1);
  }
}

const already = CUSTOMER_KEYWORDS_1000.length - targets.length;
targets = targets.slice(0, limit);

console.log(`=== Seed customer keywords 1000 ===`);
console.log(
  `Catalog: ${CUSTOMER_KEYWORDS_1000.length} | Đã có DB: ${already} | Sẽ xử lý: ${targets.length} | dry-run: ${dryRun ? "YES" : "NO"}`,
);
