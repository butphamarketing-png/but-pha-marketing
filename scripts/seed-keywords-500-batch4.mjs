/**
 * Seed 500 bài từ khóa long-tail batch 4 (bỏ qua slug đã có trên DB).
 * Chạy: npm run seed:keywords-500-batch4
 *       npm run seed:keywords-500-batch4 -- --dry-run
 */
import dotenv from "dotenv";
import { createClient } from "@supabase/supabase-js";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { KEYWORDS_500_BATCH4 } from "./seo-keywords-500-batch4.mjs";
...
let targets = KEYWORDS_500_BATCH4.filter((e) => !dbSlugs.has(e.slug));

if (onlySlug) {
  targets = KEYWORDS_500_BATCH4.filter((e) => e.slug === onlySlug);
  if (!targets.length) {
    console.error(`Slug "${onlySlug}" không có trong KEYWORDS_500_BATCH4.`);
    process.exit(1);
  }
}

const skippedExisting = KEYWORDS_500_BATCH4.filter((e) => dbSlugs.has(e.slug));
targets = targets.slice(0, limit);

console.log(`=== Seed 500 từ khóa batch 4 ===`);
console.log(`Batch: ${KEYWORDS_500_BATCH4.length} | DB: ${dbSlugs.size} | Mới: ${targets.length} | Đã có: ${skippedExisting.length}`);
...
for (let i = 0; i < targets.length; i++) {
  ...
  try {
    const article = upgradeArticle(row, i + 1500);
    if (dryRun) {
