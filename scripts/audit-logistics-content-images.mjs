import { LOGISTICS_ARTICLE_THUMBNAILS, validateSeoKeywordPlacement } from "./seo-article-helpers.mjs";
import { REWRITE_THIET_KE_WEBSITE_LOGISTICS_VAN_TAI } from "./seo-rewrite-thiet-ke-website-logistics-van-tai.mjs";

const ARTICLES = [REWRITE_THIET_KE_WEBSITE_LOGISTICS_VAN_TAI];
const EXPECTED_IMAGES = 5;
let failed = false;

for (const article of ARTICLES) {
  const images = [...article.content.matchAll(/src="(\/tin-tuc\/logistics\/[^"]+)"/g)].map((m) => m[1]);
  const otherTinTuc = [...article.content.matchAll(/src="(\/tin-tuc\/(?!logistics)[^"]+)"/g)].map((m) => m[1]);
  ...
  const thumb = LOGISTICS_ARTICLE_THUMBNAILS[article.slug];
...
console.log("All logistics articles pass SEO + 5 images check.");
