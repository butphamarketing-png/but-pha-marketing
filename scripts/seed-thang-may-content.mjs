...
async function upsertArticle(article) {
  ...
  const count = (article.content.match(/\/tin-tuc\/thang-may\//g) || []).length;
  console.log(
    `Updated: ${article.slug} (${count} thang-may images in content)`,
    seoCheck && !seoCheck.ok ? `[SEO: ${seoCheck.missing.join(", ")}]` : "",
  );
  await revalidateBlogAfterSeed(article.slug);
}

await upsertArticle(REWRITE_THIET_KE_WEBSITE_THANG_MAY);
console.log("Done — thang-may content seeded.");
