/** Alt từ khóa mockup — index 0 thumbnail, 1..n nội dung bài. */
...
export function getIndustryMockupContentAlt(pool, index) {
  const alts = INDUSTRY_MOCKUP_ALTS[pool];
  if (!alts?.length) return "Thiết kế website";
  const contentAlts = alts.slice(1);
  return contentAlts[index % contentAlts.length] ?? alts[0];
}

export function getIndustryMockupThumbnailAlt(pool) {
  return INDUSTRY_MOCKUP_ALTS[pool]?.[0] ?? "Thiết kế website";
}

/** Ảnh nội dung bài — bỏ ảnh #1 (dành cho thumbnail). */
export function nicheContentImageFiles(folder, prefix, totalMockups) {
  if (totalMockups <= 1) return [];
  return Array.from({ length: totalMockups - 1 }, (_, i) => `${folder}/${prefix}-${i + 2}.png`);
}
