...
import { KHACH_SAN_ARTICLE_THUMBNAILS, khachSanThumbnailPath } from "./khach-san-images.mjs";
...
for (const [slug, entry] of Object.entries(KHACH_SAN_ARTICLE_THUMBNAILS)) {
  const imagePath = khachSanThumbnailPath(slug);
  ...
  console.log(`Updated: ${slug} → ${imagePath} (alt: ${entry.alt})`);
  await revalidateBlogAfterSeed(slug);
}

console.log("Done — khach-san thumbnails.");
