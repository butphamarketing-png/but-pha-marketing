/**
 * Seed 200 bài từ khóa long-tail (bỏ qua slug đã có trên DB).
 * Chạy: npm run seed:keywords-200
 *       npm run seed:keywords-200 -- --dry-run
 *       npm run seed:keywords-200 -- --limit=20
 */
...
let targets = KEYWORDS_200.filter((e) => !dbSlugs.has(e.slug));

if (onlySlug) {
  targets = KEYWORDS_200.filter((e) => e.slug === onlySlug);
  if (!targets.length) {
    console.error(`Slug "${onlySlug}" không có trong KEYWORDS_200.`);
    process.exit(1);
  }
}

const skippedExisting = KEYWORDS_200.filter((e) => dbSlugs.has(e.slug));
targets = targets.slice(0, limit);

console.log(`=== Seed 200 từ khóa ===`);
console.log(`Batch: ${KEYWORDS_200.length} | DB: ${dbSlugs.size} | Mới: ${targets.length} | Đã có: ${skippedExisting.length}`);
if (skippedExisting.length && skippedExisting.length <= 15) {
...
for (let i = 0; i < targets.length; i++) {
  ...
  try {
    const article = upgradeArticle(row, i + 200);
    if (dryRun) {
