...
import {
  facebookDesignAssetFromCustomer,
  mergeFacebookDesignFromCustomer,
...
export async function POST(request: Request) {
  try {
    ...
    if (customer.platform !== "facebook" || customer.service !== "thiet-ke") {
      return NextResponse.json(
        { ok: false, error: "Chỉ đồng bộ được dịch vụ Facebook → Thiết Kế." },
        { status: 400 },
      );
    }

    const { assets } = await loadFacebookDesignAssets();
    const index = assets.findIndex((row) => row.customerRecordId === customer.id);
    const asset =
      index >= 0
        ? mergeFacebookDesignFromCustomer(assets[index], customer)
        : facebookDesignAssetFromCustomer(customer);

    const next = [...assets];
    if (index >= 0) next[index] = sanitizeFacebookDesignAsset(asset, index);
    else next.unshift(sanitizeFacebookDesignAsset(asset, next.length));

    await saveFacebookDesignAssets(next);
    const saved = next.find((row) => row.customerRecordId === customer.id) ?? asset;
    return NextResponse.json({ ok: true, asset: saved });
  } catch (error) {
    console.error("POST /api/facebook-design-assets/sync failed", error);
    return NextResponse.json({ ok: false, error: "Không thể đồng bộ thiết kế Facebook." }, { status: 500 });
  }
}
