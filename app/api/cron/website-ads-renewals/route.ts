...
import { shouldSendWebsiteAdsExpiryEmail } from "@/lib/website-ads-assets";
import { loadWebsiteAdsAssets, saveWebsiteAdsAssets } from "@/lib/website-ads-assets-store";
import { sendWebsiteAdsExpiryEmail } from "@/lib/website-ads-email";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    for (let i = 0; i < assets.length; i++) {
      ...
      if (!shouldSendWebsiteAdsExpiryEmail(asset)) continue;

      const outcome = await sendWebsiteAdsExpiryEmail(asset);
      if (outcome.sent) {
    ...
    console.error("[cron/website-ads-renewals] failed", error);
