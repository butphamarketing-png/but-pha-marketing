...
import { shouldSendFacebookAdsExpiryEmail } from "@/lib/facebook-ads-assets";
import { loadFacebookAdsAssets, saveFacebookAdsAssets } from "@/lib/facebook-ads-assets-store";
import { sendFacebookAdsExpiryEmail } from "@/lib/facebook-ads-email";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    for (let i = 0; i < assets.length; i++) {
      ...
      if (!shouldSendFacebookAdsExpiryEmail(asset)) continue;

      const outcome = await sendFacebookAdsExpiryEmail(asset);
      if (outcome.sent) {
    ...
    console.error("[cron/facebook-ads-renewals] failed", error);
