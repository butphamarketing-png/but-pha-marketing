...
import { shouldSendDomainExpiryEmail } from "@/lib/domain-assets";
import { loadDomainAssets, saveDomainAssets } from "@/lib/domain-assets-store";
import { sendDomainExpiryEmail } from "@/lib/domain-email";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const results: Array<{ id: string; domainName: string; sent: boolean; channel: string }> = [];

    for (let i = 0; i < assets.length; i++) {
      const asset = assets[i];
      if (!shouldSendDomainExpiryEmail(asset)) continue;

      const outcome = await sendDomainExpiryEmail(asset);
      if (outcome.sent) {
    ...
    console.error("[cron/domain-renewals] failed", error);
