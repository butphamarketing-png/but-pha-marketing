...
import { processAdminRenewalReminders } from "@/lib/admin-renewal-reminders";
...
export async function GET(request: Request) {
  ...
  try {
    const result = await processAdminRenewalReminders();
    return NextResponse.json(result);
  } catch (error) {
    console.error("[cron/admin-renewal-reminders] failed", error);
