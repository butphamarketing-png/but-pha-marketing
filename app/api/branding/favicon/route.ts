...
async function loadBranding() {
  try {
    ...
    return favicon || logo || DEFAULT_FAVICON_PATH;
  } catch (error) {
    console.error("[branding/favicon] Failed to load branding", error);
...
export async function GET(request: Request) {
  ...
  if (source.startsWith("http://") || source.startsWith("https://")) {
    try {
      ...
      console.error("[branding/favicon] Failed to fetch remote favicon", error);
    }
  }
