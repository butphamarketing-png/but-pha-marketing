...
export const KHACH_SAN_IMAGE_ALTS: readonly string[] = [
...
export type KhachSanThumbEntry = {
  file: string;
  keywordsMain: string;
  alt: string;
};

export const KHACH_SAN_ARTICLE_THUMBNAILS: Record<string, KhachSanThumbEntry> = {
...
export function resolveKhachSanArticleSlug(slug?: string): string | null {
  if (!slug) return null;
...
export function getKhachSanThumbnailPath(slug?: string): string | null {
  const resolved = resolveKhachSanArticleSlug(slug);
  const entry = resolved ? KHACH_SAN_ARTICLE_THUMBNAILS[resolved] : undefined;
  return entry ? `${KHACH_SAN_DIR}/${entry.file}` : null;
}

export function getKhachSanThumbnailAlt(slug?: string): string | null {
  const resolved = resolveKhachSanArticleSlug(slug);
  const entry = resolved ? KHACH_SAN_ARTICLE_THUMBNAILS[resolved] : undefined;
  return entry?.alt ?? null;
}

export function getKhachSanContentImageAlt(index: number): string {
