/** Alt text mockup website nhà hàng — public/tin-tuc/nha-hang/ */
export const NHA_HANG_IMAGE_ALTS: readonly string[] = [
...
export type NhaHangThumbEntry = {
  file: string;
  keywordsMain: string;
  alt: string;
};

export const NHA_HANG_ARTICLE_THUMBNAILS: Record<string, NhaHangThumbEntry> = {
...
export function resolveNhaHangArticleSlug(slug?: string): string | null {
  if (!slug) return null;
...
export function getNhaHangThumbnailPath(slug?: string): string | null {
  const resolved = resolveNhaHangArticleSlug(slug);
  const entry = resolved ? NHA_HANG_ARTICLE_THUMBNAILS[resolved] : undefined;
  return entry ? `${NHA_HANG_DIR}/${entry.file}` : null;
}

export function getNhaHangThumbnailAlt(slug?: string): string | null {
  const resolved = resolveNhaHangArticleSlug(slug);
  const entry = resolved ? NHA_HANG_ARTICLE_THUMBNAILS[resolved] : undefined;
  return entry?.alt ?? null;
}

export function getNhaHangContentImageAlt(index: number): string {
