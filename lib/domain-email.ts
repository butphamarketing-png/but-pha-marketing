...
export async function sendDomainExpiryEmail(asset: DomainAsset) {
  const { user, pass } = getMailConfig();
  if (!user || !pass || !asset.customerEmail.trim()) {
    return { sent: false, channel: "skipped" as const };
  }

  const { subject, text } = buildDomainExpiryEmail(asset);
  ...
  try {
    ...
    console.error("[domain-email] send failed", error);
