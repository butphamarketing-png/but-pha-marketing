 ...
 * Kết hợp: tên thân thiện + ~1.100 mã VSIC (masothue.com)
 */

import { MASOTHUE_INDUSTRIES, masothueToKeywords } from "./masothue-industries";
...
/** Tên ngành thân thiện — ưu tiên hiển thị khi tìm kiếm */
const CURATED_SUGGESTIONS: IndustrySuggestion[] = [
...
function buildMasothueSuggestions(): IndustrySuggestion[] {
  const curatedLabels = new Set(CURATED_SUGGESTIONS.map((s) => normalize(s.label)));
  return MASOTHUE_INDUSTRIES.filter((item) => !curatedLabels.has(normalize(item.name))).map((item) => ({
    label: item.name,
...
export function searchIndustrySuggestions(query: string, limit = 12): IndustrySuggestionResult[] {
  ...
  const scored = INDUSTRY_SUGGESTIONS.map((suggestion) => ({
    ...
    .sort((a, b) => b.score - a.score || (a.curated ? -1 : 1) - (b.curated ? -1 : 1) || a.label.localeCompare(b.label, "vi"));
...
export function findIndustrySuggestion(text: string): IndustrySuggestion | null {
  ...
  const exact = INDUSTRY_SUGGESTIONS.find(
    (s) =>
      normalize(s.label) === q ||
      s.keywords.some((k) => normalize(k) === q) ||
...
export function getMasothueIndustryCount() {
  return MASOTHUE_INDUSTRIES.length;
}
