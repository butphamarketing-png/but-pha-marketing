...
export const WEBSITE_ADS_ASSETS_KEY = "website_ads_assets";

export type WebsiteAdsStatus = "unverified" | "active" | "expired";

export type WebsiteAdsAsset = {
  id: string;
  ...
  status: WebsiteAdsStatus;
  lastExpiryEmailAt: string | null;
  createdAt: string;
  updatedAt: string;
};

export const WEBSITE_ADS_STATUS_OPTIONS: { key: WebsiteAdsStatus; label: string }[] = [
...
export function renewWebsiteAdsAsset(asset: WebsiteAdsAsset): WebsiteAdsAsset {
...
export function websiteAdsAssetFromCustomer(customer: CustomerRecord): WebsiteAdsAsset {
  const now = new Date().toISOString();
  const sellPrice =
...
export function mergeWebsiteAdsFromCustomer(
  existing: WebsiteAdsAsset,
  customer: CustomerRecord,
): WebsiteAdsAsset {
  const fromCustomer = websiteAdsAssetFromCustomer(customer);
  return {
    ...existing,
...
export type WebsiteAdsRowTone = "normal" | "warning" | "danger";

export function websiteAdsRowTone(asset: WebsiteAdsAsset, reference = new Date()): WebsiteAdsRowTone {
...
export function sortWebsiteAdsAssets(assets: WebsiteAdsAsset[], reference = new Date()): WebsiteAdsAsset[] {
...
export function shouldSendWebsiteAdsExpiryEmail(asset: WebsiteAdsAsset, reference = new Date()) {
  const days = daysUntilExpiry(asset.expireDate, reference);
...
export function buildWebsiteAdsExpiryEmail(asset: WebsiteAdsAsset) {
  ...
  const expiryLabel = asset.expireDate
    ...
    : "";
  return {
    subject: `[Bứt Phá Marketing] Quảng cáo website sắp hết hạn (HĐ: ${asset.contractCode})`,
    text: `Xin chào ${asset.customerName || "Anh/Chị"},

Gói quảng cáo website "${asset.packageLabel || "đang dùng"}" (HĐ: ${asset.contractCode}) sẽ hết hạn vào ${expiryLabel} (còn ${days} ngày).
...
export function sanitizeWebsiteAdsAsset(raw: unknown, index = 0): WebsiteAdsAsset {
  const now = new Date().toISOString();
  const item = raw && typeof raw === "object" ? (raw as Record<string, unknown>) : {};
  const statusRaw = typeof item.status === "string" ? item.status : "unverified";
  const status: WebsiteAdsStatus =
    statusRaw === "active" || statusRaw === "expired" || statusRaw === "unverified"
      ? statusRaw
      : "unverified";

  return {
    id: typeof item.id === "string" && item.id.trim() ? item.id : `webads-${Date.now()}-${index}`,
