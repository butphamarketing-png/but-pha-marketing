...
export async function sendFacebookAdsExpiryEmail(asset: FacebookAdsAsset) {
  const { user, pass } = getMailConfig();
  if (!user || !pass || !asset.customerEmail.trim()) {
    return { sent: false, channel: "skipped" as const };
  }

  const { subject, text } = buildFacebookAdsExpiryEmail(asset);
  ...
  try {
    ...
    console.error("[facebook-ads-email] send failed", error);
