...
import {
  FACEBOOK_ADS_ASSETS_KEY,
  sanitizeFacebookAdsAsset,
  type FacebookAdsAsset,
} from "@/lib/facebook-ads-assets";

type Payload = { entries: FacebookAdsAsset[] };
type CustomerPayload = { entries: CustomerRecord[] };

export async function loadFacebookAdsAssets(): Promise<{ assets: FacebookAdsAsset[]; serverOk: boolean }> {
  ...
  try {
    const { data, error } = await supabase
      .from("site_settings")
      .select("value")
      .eq("key", FACEBOOK_ADS_ASSETS_KEY)
      .maybeSingle();

    if (!error && Array.isArray((data?.value as Payload | null)?.entries)) {
      entries = (data?.value as Payload).entries as unknown[];
    } else if (error) {
      console.error("Facebook ads assets load failed:", error);
      serverOk = false;
    }
  } catch (error) {
    console.error("Facebook ads assets load error:", error);
    serverOk = false;
  }

  return { assets: entries.map(sanitizeFacebookAdsAsset), serverOk };
}

export async function saveFacebookAdsAssets(assets: FacebookAdsAsset[]) {
  const supabase = createServerClient();
  const { error } = await supabase
    .from("site_settings")
    .upsert({ key: FACEBOOK_ADS_ASSETS_KEY, value: { entries: assets } }, { onConflict: "key" });
  if (error) throw error;
}

export async function syncCustomerDatesFromFacebookAds(asset: FacebookAdsAsset) {
