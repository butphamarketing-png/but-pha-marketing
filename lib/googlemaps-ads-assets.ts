...
export const GOOGLEMAPS_ADS_ASSETS_KEY = "googlemaps_ads_assets";

export type GoogleMapsAdsStatus = "unverified" | "active" | "expired";

export type GoogleMapsAdsAsset = {
  id: string;
  ...
  status: GoogleMapsAdsStatus;
  lastExpiryEmailAt: string | null;
  createdAt: string;
  updatedAt: string;
};

export const GOOGLEMAPS_ADS_STATUS_OPTIONS: { key: GoogleMapsAdsStatus; label: string }[] = [
...
export function renewGoogleMapsAdsAsset(asset: GoogleMapsAdsAsset): GoogleMapsAdsAsset {
...
export function googleMapsAdsAssetFromCustomer(customer: CustomerRecord): GoogleMapsAdsAsset {
  const now = new Date().toISOString();
  const sellPrice =
...
export function mergeGoogleMapsAdsFromCustomer(
  existing: GoogleMapsAdsAsset,
  customer: CustomerRecord,
): GoogleMapsAdsAsset {
  const fromCustomer = googleMapsAdsAssetFromCustomer(customer);
  return {
    ...existing,
...
export type GoogleMapsAdsRowTone = "normal" | "warning" | "danger";

export function googleMapsAdsRowTone(asset: GoogleMapsAdsAsset, reference = new Date()): GoogleMapsAdsRowTone {
...
export function sortGoogleMapsAdsAssets(assets: GoogleMapsAdsAsset[], reference = new Date()): GoogleMapsAdsAsset[] {
...
export function shouldSendGoogleMapsAdsExpiryEmail(asset: GoogleMapsAdsAsset, reference = new Date()) {
  const days = daysUntilExpiry(asset.expireDate, reference);
...
export function buildGoogleMapsAdsExpiryEmail(asset: GoogleMapsAdsAsset) {
  ...
  const expiryLabel = asset.expireDate
    ...
    : "";
  return {
    subject: `[Bứt Phá Marketing] Quảng cáo Google Maps sắp hết hạn (HĐ: ${asset.contractCode})`,
    text: `Xin chào ${asset.customerName || "Anh/Chị"},

Gói quảng cáo Google Maps "${asset.packageLabel || "đang dùng"}" (HĐ: ${asset.contractCode}) sẽ hết hạn vào ${expiryLabel} (còn ${days} ngày).
...
export function sanitizeGoogleMapsAdsAsset(raw: unknown, index = 0): GoogleMapsAdsAsset {
  const now = new Date().toISOString();
  const item = raw && typeof raw === "object" ? (raw as Record<string, unknown>) : {};
  const statusRaw = typeof item.status === "string" ? item.status : "unverified";
  const status: GoogleMapsAdsStatus =
    statusRaw === "active" || statusRaw === "expired" || statusRaw === "unverified"
      ? statusRaw
      : "unverified";

  return {
    id: typeof item.id === "string" && item.id.trim() ? item.id : `mapads-${Date.now()}-${index}`,
