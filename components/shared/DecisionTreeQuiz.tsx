...
export function DecisionTreeQuiz({ isOpen = false, onClose, isInline = false, platform }: DecisionTreeQuizProps) {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (!userInfo.fbLink.trim()) {
      ...
      newErrors.fbLink = "Link Facebook không hợp lệ (VD: https://facebook.com/yourpage)";
    }
    ...
    try {
      ...
      // Dispatch event to trigger scroll to pricing
  ...
  // If inline mode and not open, don't render
  if (isInline && !isOpen) return null;

  const content = (
    <div className={`relative ${isInline ? 'w-full max-w-4xl mx-auto' : 'w-full max-w-lg'} overflow-hidden ${isInline ? 'bg-transparent p-0' : 'rounded-2xl border border-primary/30 bg-card p-8 shadow-2xl shadow-primary/20'}`}>
      {!isInline && (
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 4 && (
            <motion.div key="step4" initial={{ x: 50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -50, opacity: 0 }} className="absolute inset-0">
              ...
              <p className="mb-6 text-xs text-gray-400">Chúng tôi cần thông tin chính xác để phân tích kênh của bạn và đưa ra chuẩn đoán tốt nhất.</p>
          ...
          {step === 5 && (
            <motion.div key="result" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="absolute inset-0 flex flex-col items-center justify-center text-center">
              ...
              <p className="mb-6 text-gray-300 text-sm">
                Dựa trên dữ liệu kênh Facebook của bạn, tình trạng Marketing cần bứt phá tại <strong className="text-primary">{answers[1] || platform}</strong>. <br/>
                Gói dịch vụ tối ưu đề xuất: <br/>
                <strong className="mt-2 block text-xl text-primary uppercase">Gói {answers[1] || platform} {answers[3] === "Dưới 3tr" ? "Cơ Bản" : answers[3] === "3-7tr" ? "Pro" : "VIP"}</strong>
              </p>
