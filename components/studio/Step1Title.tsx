...
export function Step1Title({ data, setData, onNext, onHistoryChange }: any) {
  ...
  const handleGenerateOutline = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw {
          message: result?.error || "Không thể tạo dàn ý AI.",
      ...
      console.error(err);
      const nextError = err?.message || "Không thể tạo dàn ý AI.";
    ...
    <div className="mx-auto max-w-2xl space-y-8">
      ...
      <div className="space-y-6 rounded-[32px] border border-slate-100 bg-slate-50 p-8">
        <div className="space-y-2">
          <label className="ml-1 text-sm font-bold text-slate-700">Nhập tiêu đề bài viết</label>
          <input
            type="text"
            value={data.title}
            onChange={(e) =>
        ...
        <p className="text-center text-xs font-medium italic text-slate-400">Gợi ý: Hãy nhập tiêu đề chứa từ khoá chính để AI phân tích tốt nhất.</p>
      </div>
    </div>
  );
}
