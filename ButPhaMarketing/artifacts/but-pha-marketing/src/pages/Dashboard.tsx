...
import { db, type Order } from "@/lib/useData";
import { LogOut, LayoutDashboard, FolderOpen, Bell, Settings, FileText, BarChart2, TrendingUp } from "lucide-react";
...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (db && db.orders) {
      db.orders.getAll().then(data => {
        const myOrders = data.filter(o => o.phone === user?.phone);
        setOrders(myOrders);
      });
    }
  }, [user]);
  ...
  return (
    <div className="flex min-h-screen bg-background text-foreground">
      ...
      <main className="flex-1 p-6 md:p-8">
            ...
            <div className="rounded-2xl border border-white/10 bg-card p-6">
              <div className="mb-6 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-500">Biểu đồ hình sin phân tích lượt tiếp cận (12 tháng)</p>
                </div>
              </div>
        ...
        {activeTab === "docs" && (
          <div className="rounded-2xl border border-white/10 bg-card p-6">
            <h2 className="mb-6 text-lg font-bold text-white">Kho Tài Liệu</h2>
            <div className="space-y-3">
              {DOCS.map((d, i) => (
                <div key={i} className="flex items-center gap-4 rounded-xl bg-white/5 p-4">
                  <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg bg-primary/20 text-xs font-bold text-primary">{d.type}</div>
                  <div className="flex-1 min-w-0">
                    <p className="truncate text-sm font-medium text-white">{d.name}</p>
                    <p className="text-xs text-gray-500">{d.date}</p>
                  </div>
                  <button className="rounded-lg border border-primary/30 px-3 py-1 text-xs font-medium text-primary hover:bg-primary/10">Tải xuống</button>
                </div>
              ))}
            </div>
          </div>
        ...
        {activeTab === "notifications" && (
          <div className="rounded-2xl border border-white/10 bg-card p-6">
            <h2 className="mb-6 text-lg font-bold text-white">Thông Báo</h2>
            <div className="space-y-3">
              {NOTIFICATIONS.map((n, i) => (
                <div key={i} className={`flex items-start gap-4 rounded-xl p-4 ${n.unread ? "bg-primary/10 border border-primary/20" : "bg-white/5"}`}>
                  <div className={`mt-1 h-2 w-2 flex-shrink-0 rounded-full ${n.unread ? "bg-primary" : "bg-gray-600"}`} />
                  <div>
